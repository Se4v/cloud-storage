...
CREATE TABLE `file_entry` (
    ...
    `status`        tinyint unsigned NOT NULL DEFAULT 1 COMMENT '状态:1-未删除; 2-已删除; 3-永久删除;',
...
CREATE TABLE `file_share` (
    ...
    `entry_type`        tinyint unsigned NOT NULL DEFAULT 1 COMMENT '条目类型:1-文件; 2-文件夹',
    ...
    `link_type`         tinyint unsigned NOT NULL DEFAULT 1 COMMENT '分享类型:1-公开链接; 2-加密链接',
...
CREATE TABLE `sys_role` (
    ...
    `type`          tinyint unsigned NOT NULL DEFAULT 1 COMMENT '角色类型:1-系统角色; 2-组织内角色',
    `is_enabled`    tinyint unsigned NOT NULL DEFAULT 1 COMMENT '是否启用:0-禁用; 1-启用',
    `is_deleted`    tinyint unsigned NOT NULL DEFAULT 0 COMMENT '是否删除:0-未删除; 1-已删除',
    `created_at`    datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `creator_id`    bigint unsigned NOT NULL DEFAULT 0 COMMENT '创建者ID',
...
CREATE TABLE `sys_notice` (
    ...
    `type`          tinyint unsigned NOT NULL DEFAULT 1 COMMENT '通知类型: 1-系统通知; 2-系统告警',
